...
@section('title', 'Edit Opportunity - V-PeSDI PLWDs Database')
...
<div class="card">
    <div class="card-body">
        <form action="{{ route('admin.opportunities.update', $opportunity->id) }}" method="POST">
            @csrf
            @method('PUT')
                ...
                <div class="col-md-4">
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('type') is-invalid @enderror" 
                            ...
                            <option value="employment" {{ old('type', $opportunity->type) == 'employment' ? 'selected' : '' }}>Employment</option>
                            <option value="training" {{ old('type', $opportunity->type) == 'training' ? 'selected' : '' }}>Training</option>
                            <option value="volunteer" {{ old('type', $opportunity->type) == 'volunteer' ? 'selected' : '' }}>Volunteer</option>
                            <option value="scholarship" {{ old('type', $opportunity->type) == 'scholarship' ? 'selected' : '' }}>Scholarship</option>
                            <option value="other" {{ old('type', $opportunity->type) == 'other' ? 'selected' : '' }}>Other</option>
                        </select>
                        @error('type')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('status') is-invalid @enderror" 
                            ...
                            <option value="inactive" {{ old('status', $opportunity->status) == 'inactive' ? 'selected' : '' }}>Inactive</option>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="url" 
                               ...
                               id="website_url" 
                               name="website_url" 
                               value="{{ old('website_url', $opportunity->website_url) }}"
                    ...
                    <div class="alert alert-info">
                        <small>
                            <strong>Views:</strong> {{ $opportunity->views }}<br>
                            <strong>Created:</strong> {{ $opportunity->created_at->format('M d, Y') }}<br>
                            <strong>Updated:</strong> {{ $opportunity->updated_at->format('M d, Y') }}
                        </small>
                    </div>
                </div>
            </div>
